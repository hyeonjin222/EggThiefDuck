...
## 🎯 현재 목표 (Current Goal)
- [ ] **경험치 및 레벨업 시스템 구현:** 골드 시스템을 XP 시스템으로 전환.
- [ ] **레벨업 강화 UI:** 뱀파이어 서바이벌 스타일의 랜덤 3종 강화 선택 시스템.
- [x] **아이템 및 드롭 시스템 구현:** 적 처치 시 경험치 및 회복 아이템 드롭.

---

## 🚧 진행 중 / 다음 작업 (To-Do)
1. `AGoldItem`을 `AExpItem`으로 리팩토링 및 경험치 로직 추가.
2. `ADuckCharacter`에 레벨, 경험치 스탯 추가 및 레벨업 체크 구현.
3. 궁극기(Q 스킬) 시스템 구현: 게이지 상승 및 TPS 시점 전환 난사 로직.
4. 적 스폰 시스템: 밤 페이즈에만 맵 외곽에서 적들이 생성되도록 구현.
